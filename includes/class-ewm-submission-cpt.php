...
/**
 * Class to handle the Custom Post Type for leads generated by forms
 */
class EWM_Submission_CPT {
	...
	private $meta_fields = array(
		'modal_id',             // Source modal ID
		'form_data',            // JSON with form data
		'step_data',            // JSON with step data
	...
	public function render_details_meta_box( $post ) {
		...
		<table class="form-table">
			...
			<tr>
				...
				<td>
					...
					if ( $submission_time ) {
						...
						echo esc_html__( 'Not available', 'ewm-modal-cta' );
					}
					?>
				</td>
			</tr>
				...
				<td>
					<select name="submission_status" id="submission_status">
						...
						<option value="processed" <?php selected( $status, 'processed' ); ?>>
							<?php esc_html_e( 'Processed', 'ewm-modal-cta' ); ?>
						</option>
						<option value="archived" <?php selected( $status, 'archived' ); ?>>
							<?php esc_html_e( 'Archived', 'ewm-modal-cta' ); ?>
						</option>
					</select>
				</td>
			</tr>
			...
			<tr>
				<th scope="row">
					<label for="submission_notes"><?php esc_html_e( 'Notes', 'ewm-modal-cta' ); ?></label>
				</th>
				<td>
	...
	public function render_data_meta_box( $post ) {
		...
		<div class="ewm-submission-data">
			...
			<?php else : ?>
				<p><em><?php esc_html_e( 'No form data available.', 'ewm-modal-cta' ); ?></em></p>
			<?php endif; ?>
	...
	public function render_meta_box( $post ) {
		...
		<table class="form-table">
			<tr>
				...
				<td><?php echo esc_html( $ip_address ?: __( 'Not available', 'ewm-modal-cta' ) ); ?></td>
			</tr>
			<tr>
				<th><?php esc_html_e( 'User', 'ewm-modal-cta' ); ?></th>
				<td>
					<?php
					if ( $user_id ) {
						...
						if ( $user ) {
							...
							echo esc_html__( 'User deleted', 'ewm-modal-cta' );
						}
					} else {
			...
			<tr>
				...
				<td>
					...
					<?php else : ?>
						<?php esc_html_e( 'Not available', 'ewm-modal-cta' ); ?>
					<?php endif; ?>
				</td>
			</tr>
			<tr>
				...
				<td>
					<small><?php echo esc_html( wp_trim_words( $user_agent ?: __( 'Not available', 'ewm-modal-cta' ), 10, '...' ) ); ?></small>
				</td>
			</tr>
			<?php if ( $session_id ) : ?>
			<tr>
		...
		switch ( $column ) {
			case 'page_origin':
				...
				if ( $referer_url ) {
					...
					echo '<em style="color: #666;">' . esc_html__( 'Not available', 'ewm-modal-cta' ) . '</em>';
				}
				break;
			...
			case 'modal':
				...
				if ( $modal_id && get_post( $modal_id ) ) {
					...
					echo '<em>' . esc_html__( 'Modal deleted', 'ewm-modal-cta' ) . '</em>';
				}
				break;
			...
			case 'status':
				...
				$status_labels = array(
					...
					'processed' => __( 'Processed', 'ewm-modal-cta' ),
					'archived'  => __( 'Archived', 'ewm-modal-cta' ),
				);
			...
			foreach ( $post_ids as $post_id ) {
				...
				if ( $referer_url ) {
					...
					$new_title = sprintf(
						...
						__( 'Lead obtained from: %1$s %2$s', 'ewm-modal-cta' ),
						$page_name,
						$formatted_date
					);
		...
		if ( ! empty( $_REQUEST['ewm_updated_titles'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Display-only admin notice, no state changes
			...
			echo '<p>' . esc_html(
				sprintf(
					...
					_n(
						'Title updated for %s submission.',
						'Titles updated for %s submissions.',
						$count,
						'ewm-modal-cta'
					),
	...
	/**
	 * Detect archive pages (categories, tags, etc.)
	 *
	 * @param string $path Path de la URL
				...
				function () use ( $result ) {
					...
					echo '<p><strong>EWM:</strong> ' . esc_html(
						sprintf(
							...
							__( 'Titles updated: %1$d of %2$d submissions processed.', 'ewm-modal-cta' ),
							$result['updated'],
							$result['total']
						)
					) . '</p>';
