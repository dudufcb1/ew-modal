...
class EWM_WooCommerce_Endpoints {
	private static $allowed_coupon_fields = array(
		'id',
		'code',
		'description',
		'discount_type',
	...
	public static function get_coupons( $request ) {
		if ( ! class_exists( 'WC_Coupon' ) ) {
			return new WP_Error( 'woocommerce_missing', 'WooCommerce no está activo.', array( 'status' => 500 ) );
		}
		$args   = array(
			'posts_per_page' => 100,
		...
		foreach ( $posts as $post ) {
			...
			if ( ! empty( $extra ) ) {
				return new WP_Error( 'contract_violation', 'El endpoint generó campos fuera del contrato: ' . implode( ', ', $extra ), array( 'status' => 500 ) );
			}
			$result[] = $data;
		}
